...
LLM 配置、定时任务与失败补跑说明见 [docs/llm-config.md](docs/llm-config.md)。
...
项目统一使用 **Python 3.11**，与 FalkorDB 容器中的 Python 3.11.2 对齐；仓库根的 `.python-version` 记录了该版本。开发和测试请使用 Python 3.11 创建环境，并且代码不得使用 Python 3.12+ 语法。
...
宿主机运行 FalkorDB 集成测试时，请显式覆盖 `FALKORDB_HOST=127.0.0.1`；`.env` 中的 `falkordb-memory` 是 Docker 网络内的主机名，宿主机无法解析。
